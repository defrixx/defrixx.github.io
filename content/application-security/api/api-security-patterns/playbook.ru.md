...
- для моделирования угроз используйте [плейбук по моделированию угроз](../../../review/threat-modeling/playbook.ru.md);
...
SOAP применяется в legacy, enterprise, банковских, государственных и B2B-интеграциях, где важны формальные контракты, XML Schema, WS-* расширения и совместимость с существующими платформами. В production такие API часто стоят перед критичными backend-системами, поэтому parser hardening, подписи, fault redaction и сегментация важны не меньше, чем сама аутентификация партнера.
...
gRPC часто применяется для внутреннего service-to-service взаимодействия, low-latency бэкенд-коммуникаций и streaming. В production его нельзя считать безопасным только потому, что он “внутренний”: нужны TLS/mTLS, workload identity, method-level authorization, deadlines, message limits и контроль server reflection.
...
- Чрезмерно включенный server reflection: reflection раскрывает список services, methods и message types клиентам, которым это не нужно в production. Для атакующего это ускоряет reconnaissance и помогает строить корректные payloads без доступа к proto-файлам.
