...
- Использование уязвимых/неподдерживаемых зависимостей с `CVE` (Common Vulnerabilities and Exposures): известная дыра остается доступной в рабочей среде. Пример: старая библиотека журналирования позволяет RCE по публичному exploit.
...
Приоритет:
- Базовая severity: `High`; повышайте до `Critical`, если компрометация может затронуть подписанные релизные артефакты, CI secrets, deploy credentials рабочей среды или широко используемые packages/images.

Рабочие настройки:
- Релизное развертывание использует immutable artifact references (`sha256` digest для images) и отклоняет mutable tags вроде `latest`.
...
Негативные тесты:
- Deploy по mutable tag отклоняется в рабочей среде.
...
Приоритет:
- Базовая severity: `High`; повышайте до `Critical` для unauthenticated RCE, инъекции в данные рабочих сред stores, command execution или cross-tenant data extraction.
