...
- Каждая user, tenant, support, admin и service operation имеет явную запись политики: actor, action, resource, tenant, context.
...
- Некоторым legacy integrations нужны более слабые настройки; оформляйте их как exceptions с owner, компенсирующими мерами и expiry.
...
- CSP используется как defense-in-depth для XSS; output encoding и sanitization остаются основными browser-side controls.
...
- High-impact operations требуют step-up, approval, лимитов rate/velocity или dual control с учетом риска.
...
Приоритет:
- Базовая severity: `High`; повышайте до `Critical` для захвата admin-учетной записи, broken password reset, MFA bypass для high-impact actions или reusable refresh/session token compromise.
