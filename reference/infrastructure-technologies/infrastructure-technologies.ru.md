...
Registry promotion должен сохранять то, что проходило review. Копирование image из одного registry в другой может изменить repository reference и иногда top-level digest, если меняется copied object, media type или shape index. Поэтому review evidence должно фиксировать source и destination references, точный digest, который развернут, набор platform manifests и subject подписи/provenance, принятый policy.
...
- Image verification policies для digest, signature и attestations на рабочий workloads.
...
В рабочих средах Terraform/OpenTofu обычно запускается из CI/CD или специализированной платформы, а не с ноутбука оператора. Pipeline получает short-lived credentials через OIDC/workload identity, строит plan, сохраняет его как evidence, проходит approval и применяет изменения с ограниченной role. Ручной apply допустим только как break-glass процесс с audit trail и последующей синхронизацией state.
...
#### Границы ответственности
Terraform/OpenTofu применяет изменения инфраструктуры, но не решает, безопасна ли сама архитектура. Команда отвечает за ревью модулей, provider pinning, безопасность remote state, separation of duties, least-privilege credentials, drift detection, plan/apply approvals, policy-as-code gates и восстановимость state.
...
- Short-lived cloud credentials через OIDC/workload identity вместо long-lived access keys.
...
- `content/review/release-governance/playbook.ru.md` / `playbook.en.md` — approvals, release evidence и separation of duties для инфраструктурных изменений.
...
- `content/platform-security/secrets/vault/playbook.ru.md` / `playbook.en.md` — если Terraform/OpenTofu получает credentials или secrets из Vault.
